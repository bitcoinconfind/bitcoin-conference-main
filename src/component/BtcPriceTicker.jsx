        ...
        <div className="flex md:hidden items-center justify-center gap-4 sm:gap-6 w-full cursor-pointer group px-2 sm:px-4"
          ...
          <div className="flex flex-col items-center gap-1">
            <span className="text-white text-[10px] sm:text-xs font-bold">
              <span className="text-[#E68A00]">BTC</span><span className="!text-white">/</span><span className="!text-white">{showInr ? 'INR' : 'USD'}</span>
            </span>
            ...
            {btcData.loading ? (
              ...
              <div className="relative w-48 sm:w-64 flex justify-center">
                {/* USD Price */}
                <span className={`text-[#E68A00] font-bold text-2xl sm:text-3xl transition-all duration-500 transform ${showInr ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'
                  }`}>
                  {formatPrice(btcData.price, 'USD')}
                </span>

                {/* INR Price */}
                <span className={`absolute top-0 left-0 w-full text-center text-[#E68A00] font-bold text-2xl sm:text-3xl transition-all duration-500 transform ${showInr ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'
                  }`}>
          ...
          <div className="flex flex-col items-center gap-1 -ml-6">
            ...
            {btcData.loading ? (
              ...
              <div className="relative w-40 flex justify-center">
                {/* USD Price */}
                <span className={`text-[#E68A00] font-bold text-2xl transition-all duration-500 transform ${showInr ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'
                  }`}>
                  {formatPrice(btcData.price, 'USD')}
                </span>

                {/* INR Price */}
                <span className={`absolute top-0 left-0 w-full text-center text-[#E68A00] font-bold text-2xl transition-all duration-500 transform ${showInr ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'
                  }`}>
                  {formatPrice(getInrPrice(), 'INR')}
                </span>
              </div>
